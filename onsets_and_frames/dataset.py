...
class PianoRollAudioDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        result['onset'] = (result['label']*(result['label'] == 5).float()).float()
        result['offset'] = (5*result['label']*(result['label'] == 1).float()).float()
        result['frame'] = (result['label']*(result['label'] > 1).float()).float()
    ...
    def load(self, audio_path, tsv_path):
        ...
        label = torch.zeros(n_steps, n_keys, dtype=torch.float32)
        ...
        for onset, offset, note, vel in midi:
            ...
            f = int(note) - MIN_MIDI
            label[left:onset_right, f] = 5
            label[onset_right:frame_right, f] = torch.as_tensor(5.0/ np.clip(np.arange(2, frame_right-onset_right+2), 1,4))
...
class Raag(PianoRollAudioDataset):

    def __init__(self, path='../repo_data/sitar_lmd_files', groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        super().__init__(path, groups if groups is not None else ['train'], sequence_length, seed, device)
    ...
    def files(self, group):
        if group not in self.available_groups():
            ...
            metadata = pd.read_excel(os.path.join(self.path, 'group_split.xlsx'))
            files = sorted([(os.path.join(self.path, row['audio_filename'].replace('.wav', '.flac')),
                             os.path.join(self.path, row['midi_filename'])) for index, row in metadata.iterrows() if row['split'] == group])
